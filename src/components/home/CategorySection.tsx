...
import { API_BASE, absImage } from "../../lib/api";
...
export default function CategorySection() {
  ...
  const visible = useMemo(() => {
    ...
    return [...others.slice(0, 11), otherVeg].filter(Boolean).slice(0, 12) as Category[];
  }, [cats]);
      ...
      <div className="mx-auto w-full max-w-7xl px-4">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 auto-rows-fr">
          {loading && Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-full flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white animate-pulse shadow-sm">
          ...
          {!loading && !err && visible.map((c) => {
            ...
            const img = absImage(c.image_path);

            return (
              <article key={c.id} className="group relative h-full overflow-hidden rounded-2xl border border-gray-200 bg-white transition-all hover:shadow-xl hover:border-[#27ae60]/30">
